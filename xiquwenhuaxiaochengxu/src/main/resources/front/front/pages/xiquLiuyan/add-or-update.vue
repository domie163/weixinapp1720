        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.xiquLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`xiquLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.xiquId){
                this.ruleForm.xiquId = options.xiquId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.xiquLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`xiquLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`xiquLiuyan`, this.ruleForm);
                }
