	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xiquLiuyan', this.xiquLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xiquLiuyanPhoto ? this.detail.xiquLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xiquLiuyanYuyue', {
									yonghuId: _this.user.id,
									xiquLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xiquLiuyanYuyue/list');
							}
						}
					});
				},
