	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xiquCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xiquCollection', this.xiquCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xiquCollectionPhoto ? this.detail.xiquCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xiquCollectionYuyue', {
									yonghuId: _this.user.id,
									xiquCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xiquCollectionYuyue/list');
							}
						}
					});
				},
