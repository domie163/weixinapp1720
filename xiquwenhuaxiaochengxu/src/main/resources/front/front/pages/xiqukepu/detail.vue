<template>
			<view style="margin: 0 30rpx;box-shadow:0 0px 0px 1rpx #a6a6a6">
				...
				<view style="font-weight: bold;font-size: 20upx;margin:0 30rpx 5rpx;"  :style="(detail.xiqukepuValue == null || detail.insertTime == null)?'text-align: center;':''">
					<text v-if="detail.xiqukepuValue">
						{{detail.xiqukepuValue}}
					</text>
					<text :style="detail.xiqukepuValue?'float: right':''">
						{{detail.insertTime}}<!-- .split(' ')[0] -->
					</text>
				</view>
				<view style="box-shadow: 0 0  0 1rpx #a6a6a6;margin: 0 30rpx"></view>
                <viwe>
					<image :src="baseUrl+detail.xiqukepuPhoto"></image>
                </viwe>
				<view class="content">
					<rich-text :nodes="detail.xiqukepuContent"></rich-text>
				</view>
			</view>
		...
		export default {
			...
			methods: {
				...
				async init() {
					...
					this.detail.xiqukepuContent = this.detail.xiqukepuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xiqukepuPhoto ? this.detail.xiqukepuPhoto.split(",") : [];
				},
