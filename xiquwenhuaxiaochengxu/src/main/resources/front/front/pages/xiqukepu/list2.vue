	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 6rpx 12rpx var(--publicSubColor)","margin":"6rpx 0rpx","borderColor":"#ccc","backgroundColor":"rgba(255, 255, 255, 1)","color":"#333","textAlign":"left","borderRadius":"40rpx 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.xiqukepuName" type="text" placeholder="戏曲科普名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('xiqukepu','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
...
<view class="news-box5"
    ...
    <view @tap="onDetailTap(item)" v-for="(item,index) in list" :key="index" class="list-item"
        ...
        <view
                ...
                class="category hide2" v-html="item.xiqukepuContent">
        </view>
        <view
...
<script>
	export default {
		...
		async onShow() {
            ...
            this.categoryList = xiqukepuTypes.data.list
				...
				let params = {
					...
					limit: mescroll.size,
							...
							if (this.categoryName != 0) {
                                params.xiqukepuTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`xiqukepu`, {
                                id:id,
                            });
