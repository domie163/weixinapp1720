			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.xiquName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="xiangqing" @tap="download(baseUrl + detail.xiquFile)">		
					<view class="shangpinxiangqing">
						<view class="box">
							戏曲文件：
						</view>
				...
				<view class="yinyuewai">
					<view class="yinyuenei">
						...
						<view class="" style="margin: 25rpx;">
							...
							<audio :src="baseUrl+detail.xiquMusic" :poster="detail.xiquPhoto"
								   :name="detail.xiquName" :action="{'method':'pause'}" controls></audio>
						</view>
					</view>
				</view>
				<!-- 视频 -->
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.xiquVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.xiquContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xiquId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getxiquCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
						...
						xiquLiuyanPingfenNumber: this.scoreData,
                        xiquLiuyanText: this.commentData,
                    }
                    await this.$api.save(`xiquLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xiqu', this.xiquId);
                    this.detail = res.data
					this.detail.xiquContent = this.detail.xiquContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xiquPhoto ? this.detail.xiquPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getxiquCollection() {
					let params = {
						page: 1,
						limit: 1,
						xiquId: this.xiquId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`xiquCollection`, params);
					if (res.data.list.length == 1) {
						let xiquCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('xiquCollection', JSON.stringify([xiquCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getxiquCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xiquCollection', {
									yonghuId: _this.user.id,
									xiquId: _this.detail.id,
									xiquCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('xiquLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						xiquId: this.xiquId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('xiqu', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xiquYuyue', {
									yonghuId: _this.user.id,
									xiquId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xiquYuyue/list');
							}
						}
					});
				},
